  ...
  componentDidMount() {
    ...
    fetch('http://localhost:5000/graphql', {
      ...
      .then(resData => {
        ...
        this.setState({ status: resData.data.getCurrentUser.status });
      })
  ...
  finishEditHandler = postData => {
    ...
    fetch('http://localhost:5000/post-image', {
      ...
      .then(fileResData => {
        ...
        let graphqlQuery = {
          query: `
          mutation CreateNewPost($postData: PostData!) {
        ...
        if (this.state.editPost) {
          graphqlQuery = {
            query: `
              mutation UpdateExistingPost($postId: ID!, $postData: PostData!) {
                editPost(postId: $postId, postData: $postData) {
                  _id
                  title
                  content
                  imageUrl
